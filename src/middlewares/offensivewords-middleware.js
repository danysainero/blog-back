...
class OffensiveWordsMiddleware {
  ...
  async OffensiveWordsChecker(req, res, next) {
    ...
    const offensivewordsFound = await OffensiveWordsIncludes.checkWordsOnComment(
    ...
    if (offensivewordsFound.length === 0) {
      ...
      const info = offensivewordsFound.map(
        (offensiveword) =>
          offensiveword.word + " con nivel " + offensiveword.level
      );
